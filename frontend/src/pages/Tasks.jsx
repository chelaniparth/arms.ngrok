...
import { Plus, Filter, MoreHorizontal, Upload, Play, CheckCircle, UserPlus, Star, Pencil, LayoutList, User, Trash2, XCircle, RotateCcw, Pause, PlayCircle } from 'lucide-react';
...
const Tasks = () => {
    ...
    const handlePause = async (e, task) => {
        e.stopPropagation();
        try {
            await api.post(`/tasks/${task.task_id}/pause`);
            handleRefresh();
        } catch (error) {
            console.error("Failed to pause task", error);
            alert(error.response?.data?.detail || "Failed to pause task");
        }
    };

    const handleResume = async (e, task) => {
        e.stopPropagation();
        try {
            await api.post(`/tasks/${task.task_id}/resume`);
            handleRefresh();
        } catch (error) {
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 font-medium">
                            <tr>
                                ...
                                {viewMode === 'my' && <th className="px-6 py-4 whitespace-nowrap">Quality</th>}
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {loading ? (
                                ...
                                filteredTasks.map((task) => {
                                    ...
                                    const canComplete = (task.status === 'In Progress' || task.status === 'Paused') && (isAssignedToMe || currentUser?.role === 'admin');
                                    const canUnpick = task.status === 'In Progress' && (isAssignedToMe || isAdminOrManager);
                                    const canPause = task.status === 'In Progress' && (isAssignedToMe || isAdminOrManager);
                                    const canResume = task.status === 'Paused' && (isAssignedToMe || isAdminOrManager);
                                    const canAssign = isAdminOrManager;
                                    const canDelete = isAdminOrManager;
                                    const canReview = isAdminOrManager && task.status === 'Completed';
                                        ...
                                        <tr key={task.task_id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                                            ...
                                            {viewMode === 'my' && (
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex flex-col gap-1">
                                                        ...
                                                        {task.error_status && task.error_status !== 'None' && (
                                                            <div className="flex items-center gap-1 text-xs text-red-500 font-medium">
                                                                <XCircle size={12} />
                                                                <span>{task.error_status}</span>
                                                            </div>
                                                        )}
                                                        {!task.rating && !task.error_status && <span className="text-slate-400 text-xs">-</span>}
                                                    </div>
                                                </td>
                                            )}
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    {canPause && (
                                                        <button
                                                            onClick={(e) => handlePause(e, task)}
                                                            className="inline-flex items-center px-2.5 py-1.5 text-xs font-medium text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 hover:bg-amber-100 dark:hover:bg-amber-900/50 rounded-lg transition-colors"
                                                            title="Pause Task"
                                                        >
                                                    ...
                                                    {canResume && (
                                                        <button
                                                            onClick={(e) => handleResume(e, task)}
                                                            className="inline-flex items-center px-2.5 py-1.5 text-xs font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-lg transition-colors"
                                                            title="Resume Task"
                                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right relative">
                                                ...
                                                {activeDropdown === task.task_id && (
                                                    <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-100 dark:border-slate-700 z-10 py-1">
                                                        ...
                                                        {canReview && (
                                                            <button
                                                                onClick={(e) => openReviewModal(e, task)}
