    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            fetchedUsers.forEach(user => {
                const userTasks = allTasks.filter(t => t.assigned_user_id === user.id && t.status === 'Completed');
                const ratedTasks = userTasks.filter(t => t.rating);
                const totalRated = ratedTasks.reduce((sum, t) => sum + t.rating, 0);
                const avgRating = ratedTasks.length > 0 ? (totalRated / ratedTasks.length).toFixed(1) : '-';

                const errorFreeTasks = userTasks.filter(t => !t.error_status || t.error_status === 'None');
                const errorFreeRate = userTasks.length > 0 ? Math.round((errorFreeTasks.length / userTasks.length) * 100) : '-';

                stats[user.id] = { avgRating, errorFreeRate, totalCompleted: userTasks.length };
            });
            setUserStats(stats);
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 text-slate-500 font-medium">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-center">Error Free %</th>
                                ...
                                users.map((user) => (
                                    <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-center font-medium text-slate-700">
                                            {(['admin', 'manager'].includes(currentUser?.role) || currentUser?.id === user.id) ? (
                                                <>
                                                    {userStats[user.id]?.avgRating || '-'}
                                                    {userStats[user.id]?.avgRating !== '-' && <span className="text-slate-400 text-xs ml-1">/5</span>}
                                                </>
                                            ) : (
                                        ...
                                        <td className="px-6 py-4 text-center font-medium text-slate-700">
                                            {(['admin', 'manager'].includes(currentUser?.role) || currentUser?.id === user.id) ? (
                                                userStats[user.id]?.errorFreeRate !== '-' ? (
                                                    <span className={clsx(
                                                        userStats[user.id]?.errorFreeRate >= 95 ? "text-emerald-600" :
                                                            userStats[user.id]?.errorFreeRate >= 80 ? "text-amber-600" : "text-red-600"
                                                    )}>
                                                        {userStats[user.id]?.errorFreeRate}%
                                                    </span>
                                                ) : '-'
