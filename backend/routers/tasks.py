...
def log_history(db: Session, task_id: int, user_id: Optional[str], field_name: str, old_value: Optional[str], new_value: Optional[str], action: str, notes: Optional[str] = None):
...
def pick_task(task_id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(auth.get_current_user)):
    ...
    if not db_task.tier1_started_at:
        db_task.tier1_started_at = datetime.now()
        
    # Log History
    log_history(db, task_id, current_user.id, "status", old_status, "In Progress", "status_changed")
    log_history(db, task_id, current_user.id, "assigned_user_id", None, str(current_user.id), "assigned", f"Assigned to {current_user.full_name}")
...
def unpick_task(task_id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(auth.get_current_user)):
    ...
    # Revert to Pending
    old_status = db_task.status
    old_user = db_task.assigned_user_id
    ...
    # Log History
    log_history(db, task_id, current_user.id, "status", old_status, "Pending", "status_changed")
    log_history(db, task_id, current_user.id, "assigned_user_id", str(old_user), None, "unassigned", "Task unpicked")
...
def complete_task(task_id: int, completion_data: schemas.TaskComplete, db: Session = Depends(get_db), current_user: schemas.User = Depends(auth.get_current_user)):
    ...
    # This is implicitly handled if we use start/end times and subtract total_paused_duration_seconds
    # But we don't store "duration" column, we calculate it on fly or in analytics.
    # However, for SLA, we might want to know.
    
    # If task was paused when completed (unlikely but possible), resume it first to stop the pause timer?
    # Or just assume it's done.
    if db_task.status == "Paused":
        # If completing a paused task, add the final segment to total_paused
        if db_task.last_paused_at:
            # Ensure both datetimes are timezone-aware
            now = datetime.now(db_task.last_paused_at.tzinfo) if db_task.last_paused_at.tzinfo else datetime.now()
            paused_duration = (now - db_task.last_paused_at).total_seconds()
            db_task.total_paused_duration_seconds += int(paused_duration)
        db_task.last_paused_at = None

    # Update Task
    old_status = db_task.status
    db_task.status = "Completed"
    if not db_task.tier1_completed_at:
        db_task.tier1_completed_at = datetime.now()
        
    # Log History
    log_history(db, task_id, current_user.id, "status", old_status, "Completed", "status_changed")
...
def pause_task(task_id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(auth.get_current_user)):
    db_task = db.query(models.Task).filter(models.Task.task_id == task_id).first()
    if not db_task:
        raise HTTPException(status_code=404, detail="Task not found")
    ...
    # Check permissions
    if db_task.assigned_user_id != current_user.id and current_user.role not in [models.UserRole.admin, models.UserRole.manager]:
        raise HTTPException(status_code=403, detail="You can only pause tasks assigned to you")

    if db_task.status != "In Progress":
        raise HTTPException(status_code=400, detail="Only 'In Progress' tasks can be paused")

    db_task.status = "Paused"
    db_task.last_paused_at = datetime.now()
    
    # Log History
    log_history(db, task_id, current_user.id, "status", "In Progress", "Paused", "status_changed", "Task Paused")
    
    db.commit()
    db.refresh(db_task)
    return {"message": "Task paused", "task": db_task}

@router.post("/{task_id}/resume")
def resume_task(task_id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(auth.get_current_user)):
    db_task = db.query(models.Task).filter(models.Task.task_id == task_id).first()
    if not db_task:
        raise HTTPException(status_code=404, detail="Task not found")
    ...
    # Check permissions
    if db_task.assigned_user_id != current_user.id and current_user.role not in [models.UserRole.admin, models.UserRole.manager]:
        raise HTTPException(status_code=403, detail="You can only resume tasks assigned to you")
    ...
    # Log History
    log_history(db, task_id, current_user.id, "status", "Paused", "In Progress", "status_changed", "Task Resumed")
    
    db.commit()
    db.refresh(db_task)
    return {"message": "Task resumed", "task": db_task}

@router.put("/{task_id}/review", response_model=schemas.Task)
def review_task(task_id: int, review_data: schemas.TaskReview, db: Session = Depends(get_db), current_user: schemas.User = Depends(auth.get_current_user)):
    db_task = db.query(models.Task).filter(models.Task.task_id == task_id).first()
    if not db_task:
        raise HTTPException(status_code=404, detail="Task not found")
    
    print(f"REVIEW REQUEST: User={current_user.username}, Role={current_user.role}, Data={review_data}")
    
    # Check permissions: Only admin/manager can review
    if current_user.role not in [models.UserRole.admin, models.UserRole.manager]:
        print(f"PERMISSION DENIED: User {current_user.username} has role {current_user.role}")
        raise HTTPException(status_code=403, detail="Only admins and managers can review tasks")
    ...
    try:
        ...
        db_task.error_status = review_data.error_status
        if review_data.remarks:
            db_task.remarks = review_data.remarks
